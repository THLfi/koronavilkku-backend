...
public class PublishTokenService {
    ...
    public PublishToken generateAndStore(LocalDate symptomsOnset, String requestService, String requestUser, Optional<Boolean> symptomsExists) {
        int retryCount = 0;
        PublishToken token = generate();
        while (!dao.storeToken(token, symptomsOnset, requestService, requestUser, symptomsExists)) {
