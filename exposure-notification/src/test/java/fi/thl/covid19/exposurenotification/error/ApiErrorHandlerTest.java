...
public class ApiErrorHandlerTest {
    ...
    @Test
    public void invalidTokenIs403() throws Exception {
        String result = mockMvc.perform(get("/test/token-verification-failure"))
                .andExpect(status().isForbidden())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andReturn().getResponse().getContentAsString();
        assertErrorObject(result, HttpStatus.FORBIDDEN, Optional.of("Publish token not accepted"));
    }
